...
function temporarilyFixGradientText(elements: HTMLElement[]): () => void {
  ...
  elements.forEach((element) => {
    ...
    logger.debug(
      `Fixing gradient text for ${element.tagName}.${element.className}, using color: ${extractedColor}`
    );
  ...
  while (currentElement && depth < maxDepth) {
    ...
    if (bgColor && bgColor !== 'transparent' && bgColor !== 'rgba(0, 0, 0, 0)') {
      ...
      if (rgbaMatch) {
        ...
        if (alpha > 0.1) {
          // Consider backgrounds with > 10% opacity
          logger.debug(`Using background color from parent: ${bgColor}`);
          return bgColor;
        }
      } else {
...
export async function captureElementScreenshot(
  ...
  try {
    ...
    if (gradientTextElements.length > 0) {
      logger.debug('Found gradient text elements, applying temporary fix');
    ...
    try {
      ...
      logger.warn('CORS error during screenshot, trying with allowTaint:', corsError);
      ...
      try {
        ...
        logger.warn('Screenshot failed even with allowTaint:', taintError);
...
export async function captureScreenshotWithFallback(
  ...
  if (result.corsIssue) {
    logger.info('Retrying screenshot with fallback options');
  ...
  logger.info('Creating placeholder image for failed screenshot');

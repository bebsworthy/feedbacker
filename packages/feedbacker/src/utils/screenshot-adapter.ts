 ...
 */
export function initializeCaptureManager(
  library?: string,
  ...
  if (adapter) {
    ...
    try {
      ...
      console.warn(`[Screenshot] Failed to set library '${library}':`, error);
    }
  }
}
...
export async function captureScreenshotWithAdapters(
  ...
  try {
    ...
    console.warn('[Screenshot] Primary capture failed, trying fallback');

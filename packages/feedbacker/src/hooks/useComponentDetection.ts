...
export function useComponentDetection(): UseComponentDetectionResult {
  ...
  const detectComponent = useCallback(
    (element: HTMLElement): ComponentInfo | null => {
      ...
      try {
        ...
        endMark();
        logger.error('Component detection failed:', error);
        return null;
      }
    },
  ...
  const detectComponentOptimized = useCallback(
    (element: HTMLElement): void => {
      ...
      idleCallbackId.current = requestIdleCallback(
        (_deadline) => {
          if (_deadline.timeRemaining() > 5 || _deadline.didTimeout) {
  ...
  const handleMouseMove = useMemo(
    () =>
      throttle((event: MouseEvent) => {
        ...
        if (target) {
          logger.debug('Detecting component for:', target.tagName);
  ...
  const handleClick = useMemo(
    () => (event: MouseEvent) => {
      ...
      if (target) {
        ...
        logger.debug('Component CLICKED and selected:', componentInfo);
      }
    },
    [detectComponent]
  );
  ...
  const handleKeyDown = useCallback(
    (event: KeyboardEvent) => {
      ...
      if (event.key === 'Escape') {
        event.preventDefault();
        event.stopPropagation();
        logger.debug('ESC key pressed, deactivating');
        deactivate();
      }
    },
    [deactivate]
  ...
  const activate = useCallback(() => {
    logger.debug('activate called, current isActive:', isActive);
    if (isActive) {
      logger.debug('Already active, returning');
      return;
    }

    logger.debug('Setting isActive to true');
    setIsActive(true);
    // Cursor styling
    ...
    if (lastMousePosition.current) {
      ...
      if (elementUnderMouse) {
        logger.debug('Immediately detecting element under mouse:', elementUnderMouse.tagName);

...
import { FeedbackStore, Feedback, Draft, BrowserInfo } from './types';
...
/**
 * Get default browser info, safe for non-browser environments
 */
...
/**
 * Get current URL, safe for non-browser environments
 */
...
function normalizeFeedback(feedback: any): Feedback | null {
  try {
    ...
    const normalized: Feedback = {
      ...
      url: feedback.url ? String(feedback.url) : getCurrentUrl(),
  ...
  try {
    ...
    return {
      componentInfo: {
        ...
        element: draft.componentInfo.element,
...
function normalizeBrowserInfo(browserInfo: any): BrowserInfo {
  const defaultInfo = getDefaultBrowserInfo();
...
function migrateFrom_0_1_0(data: any): FeedbackStore | null {
  try {
    const defaultBrowserInfo = getDefaultBrowserInfo();
    ...
    return {
      ...
      feedbacks: Array.isArray(data.feedbacks)
        ? data.feedbacks.map((feedback: any) => ({
            ...
            browserInfo: feedback.browserInfo || defaultBrowserInfo
          }))
        : [],
